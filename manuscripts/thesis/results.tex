...
\item[II.] The authors in \cite{Hillar15} have applied measure-theoretic arguments to achieve $(k+1){m \choose k}$ with almost-certainty (i.e. with probability one), a factor of $m$ reduction over that for which certainty can alternatively be guaranteed here. In their case, $(k+1)$ vectors need to be allocated to each of ${m \choose k}$ distinct supports, whereas here $(k-1){m \choose k} + 1$ vectors are allocated to each of $m$ distinct supports.
