...
\IEEEPARstart{B}{lind} source separation is a classical problem in signal processing \cite{sato1975method}.
...
Find a matrix $\mathbf{B}$ and $k$-sparse $\mathbf{\overline x}_1, \ldots, \mathbf{\overline x}_N \in \mathbb{R}^{\overline m}$ with $\|\mathbf{z}_i - \mathbf{B}\mathbf{\overline x}_i\|_2 \leq \eta$ for all $i$.
\end{problem}
