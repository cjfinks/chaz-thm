...
%Hence, the lemma holds for the largest number $p \leq \bar m - (\bar m - m) d$ in the interval $1 \leq p \leq m$, which is guaranteed for $p = \bar m - (\bar m - m) d$ if $m \leq \bar m \leq (md-1) / (d-1)$. We can't have $\bar m < m$ since $\bar m \geq \sum_i d_i / d_1 = m$ by the contradiction discussed in the proof. 
\end{remark}
