...
has a stable sparse representation.  More specifically, such dictionaries are uniquely identifiable from as few as \mbox{$N = m(k-1){m \choose k} + m$} sparse linear combinations of their columns up to an error that is linear in the measurement noise (Thm.~\ref{DeterministicUniquenessTheorem}). In fact, provided $n \geq k$, %$(n, m, k)$ satisfy CS inequality \eqref{CScondition}, 
...
Fix $n, m$ and $k$ with $n \geq k < m$, % satisfying \eqref{CScondition} for $\gamma = \gamma_0$, 
...
\end{align}
%
Also, from \cite[Lem.~3.2]{Morris10}:
...
[*** TODO: Insert the final proof here ***]
\end{lemma}
\begin{proof}
...
We first claim that $\text{\rm Span}\{\mathbf{B}_{\pi(S)}\}$ and $\text{\rm Span}\{\mathbf{A}_{S}\}$ have the same dimension for all $S \in E$. To see why, note that the right-hand side of \eqref{GapUpperBound} is strictly less than one, since $r \leq 1$ and $L_2(\mathbf{A}) \leq \max_{j \in [m]}\|\mathbf{A}_j\|_2$ by \eqref{delrho}. From \eqref{dimLem}, it then follows that $|\pi(S)| \geq \dim(\text{\rm Span}\{\mathbf{B}_{\pi(S)}\}) \geq \dim(\text{\rm Span}\{\mathbf{A}_{S}\}) = |S|$, with the equality due to $\mathbf{A}$ satisfying \eqref{SparkCondition}; hence, $\dim(\text{\rm Span}\{\mathbf{B}_{\pi(S)}\}) = \dim(\text{\rm Span}\{\mathbf{A}_{S}\})$ (since $|S| = |\pi(S)|$). The columns of $\mathbf{B}_{\pi(S)}$ are thus linearly independent and by \eqref{eqdim}:% and $\mathbf{B}_i \neq \textbf{0}$ for all $i$.
...
Fix $F \subseteq E$. Since $\text{\rm Span}\{\mathbf{B}_{\cap_{S \in F}\pi(S)}\} \subseteq \cap_{S \in F} \text{\rm Span}\{\mathbf{B}_{\pi(S)}\}$, if $\cap_{S \in F} \text{\rm Span}\{\mathbf{B}_{\pi(S)}\} = \{\textbf{0}\}$, then $|\cap_{S \in F} \pi(S)| = 0$ (as $\mathbf{B}_i \neq \{\textbf{0}\}$ for all $i$) and \eqref{cond} trivially holds. On the other hand, suppose the intersection is not the zero vector. Using Lem.~\ref{SpanIntersectionLemma} and Lem.~\ref{DistanceToIntersectionLemma}, and incorporating \eqref{eq2} and \eqref{GapUpperBound}, we have:
...
\leq C_2 \varepsilon. 
\end{align}
The right-hand side in \eqref{randoml} is strictly less than one, so \eqref{dimLem} implies that $\dim(\text{\rm Span}\{\mathbf{B}_{\cap_{S \in F}\pi(S)}\}) \leq \dim(\text{\rm Span}\{\mathbf{A}_{\cap_{S \in F} S}\})$ and \eqref{cond} follows from the linear independence of the columns of $\mathbf{A}_{S}$ and $\mathbf{B}_{\pi(S)}$ for all $S \in F$.


[*** TODO: Check this last move here: ***]
Applying Lem.\ref{NonEmptyLemma}, there is a $J \subseteq [m]$ and injective map $\bar{\pi}: J \to [\bar m]$ with $\|\mathbf{A}\mathbf{e}_j - \bar{c}_j \mathbf{B}\mathbf{e}_{\bar \pi(j)}\|_2 \leq \varepsilon < L_2(\mathbf{A})/C_1$, for some $\bar{c}_j \in \mathbb R$ and all $j \in J$.
%Now, fix $\ell \in [m]$. Since $E$ satisfies the singleton intersection property, $\{\ell\} = \cap F(\ell)$ for the star $F(\ell) \subseteq E$. By \eqref{cond}, we have that $\cap_{S \in F(\ell)} \pi(S)$ is either empty or it contains a single element. Lem.~\ref{NonEmptyLemma} ensures the latter case is the only possibility. Thus, the association $\ell \mapsto \cap_{S \in F(\ell)} \pi(S)$ defines a map $\bar \pi: [m] \to [m]$ and $\dim(\text{\rm Span}\{\mathbf{B}_{\bar \pi(\ell)}\}) = 1$, since $\mathbf{B}_i \neq \textbf{0}$ for all $i$. By \eqref{eqdim}, it follows from \eqref{randoml} that $d\left( \text{\rm Span}\{\mathbf{A}_\ell\}, \text{\rm Span}\{ \mathbf{B}_{\bar \pi(\ell)} \} \right) \leq C_2 \varepsilon$. Since $\ell$ is arbitrary, fixing $\bar \varepsilon = C_2\delta$ it follows from \eqref{randoml} that for every basis vector $\mathbf{e}_\ell \in \mathbb{R}^m$ there exists some $c'_\ell \in \mathbb{R}$ such that $\|\mathbf{A}\mathbf{e}_\ell - c'_\ell \mathbf{B}\mathbf{e}_{\bar \pi(\ell)}\|_2 \leq \bar \varepsilon < L_2(\mathbf{A})$. This is exactly the supposition in \eqref{1D} (letting $c_i = \|\mathbf{A}_i\|_2^{-1}$ for all $i$) and 
The result now follows from the $k=1$ case in Sec.~\ref{DUT}.
\end{proof}

%; applying Lem.\ref{NonEmptyLemma} finishes the proof.
...
% We sketch the argument, leaving the details to the reader.  %\cite[Sec.~IV]{Hillar15}
Let $M$ be the matrix with columns $\mathbf{A}\mathbf{x}_i$, $i \in [N]$.  Consider the following polynomial in the entries of $\mathbf{A}$, $\mathbf{x}_i$:
\begin{align*}
g(\mathbf{A}, \{\mathbf{x}_i\}_{i=1}^N) = \prod_{S \in {[n] \choose k}} \sum_{S' \in {[N] \choose k}} (\det M_{S',S})^2,
\end{align*}
with notation as in Sec.~\ref{results}.  It can be checked that when $g$ is nonzero for a substitution of real numbers for the indeterminates, all of the genericity requirements on $\mathbf{A}$ and $\mathbf{x}_i$ in our proofs of stability in Thm.~\ref{DeterministicUniquenessTheorem} are satisfied (in particular, the spark condition on $\mathbf{A}$). % The statement of the theorem now follows directly.
\end{proof}
