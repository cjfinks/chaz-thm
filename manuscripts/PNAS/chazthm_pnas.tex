...
%Fixing $\mathbf{y} \in \text{\rm Span}\{ \mathbf{A}_S \}$, there exists $\mathbf{c} = (c_1, \ldots, c_k) \in \mathbb{R}^k$ such that $\mathbf{y} = \mathbf{AX}_{J}\mathbf{c}$. Setting \mbox{$\mathbf{\bar{y}} = \mathbf{B\bar{X}}_{J}\mathbf{c} \in \text{\rm Span}\{\mathbf{B}_{\bar S}\}$}, we have:
%\begin{align*}
%\|\mathbf{y} - \mathbf{\bar{y}}\|_2 
%&= \|\sum_{i=1}^k c_i(\mathbf{AX}_J - \mathbf{B\bar{X}}_J)_i\|_2
%\leq \varepsilon \sum_{i=1}^k |c_i| \\
%&\leq \varepsilon \sqrt{k}  \|\mathbf{c}\|_2 
