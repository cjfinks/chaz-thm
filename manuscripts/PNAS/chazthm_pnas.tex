...
\significancestatement{Many naturally occurring signals (visual scenery, speech, electroencephalograms, etc.) for which we lack theoretically motivated formal models can be usefully characterized as parsimonious combinations of elementary waveforms drawn from a large `dictionary'. We give general conditions guaranteeing when such dictionaries are uniquely and stably determined by enough data. The result provides theoretical grounding for the application of dictionary learning methods to problems of blind source separation, and may help to explain the observed universality of emergent representations in sparse coding models of some natural phenomena.
...
One common assumption is that each of $N$ observed $n$-dimensional signals is a linear combination of at most $k$ elementary waveforms drawn from some unknown ``dictionary'' of size $m$, typically with $k < m \ll N$ (see \cite{Zhang15} for a comprehensive review of this and related models).
...
Previous theoretical work on Prob.~\ref{InverseProblem} has adressed only the noiseless case $\eta =0$ (e.g., \cite{li2004analysis, Georgiev05, Aharon06, Hillar15}), showing that a solution (when it exists) is indeed unique provided the $\mathbf{x}_i$ are sufficiently diverse and the generating matrix $\mathbf{A}$ satisfies the \textit{spark condition} from compressed sensing (CS):
...
Finally, we present several applications in Discussion Sec.~\ref{Discussion}. These include: 1) a sufficient diversity of sparse codes sent through a random linear compressive channel are identifiable and 2) an explanation for universal representation of natural signals in sparse coding models from neuroscience.

\section{Results}\label{Results}

Before stating our main results, we first identify the combinatorial criteria on support sets of generating codes allowing for stable sparse 
...
1) Every matrix $\mathbf{B}$ and $k$-sparse $\mathbf{\bar x}_1, \ldots, \mathbf{\bar x}_N \in \mathbb{R}^{\bar m}$ with  \mbox{$\|\mathbf{A}\mathbf{x}_i - \mathbf{B}\mathbf{\bar x}_i\|_2 \leq \varepsilon$} for all $i$ must have $\bar m \geq m$ and, provided $p = \bar m - \ell(\bar m - m)$ is positive, necessarily satisfy:
