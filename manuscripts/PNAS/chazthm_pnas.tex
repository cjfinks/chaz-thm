...
\dropcap{B}lind source separation is a classic inverse problem in signal processing \cite{sato1975method}.
One constraint commonly imposed is that each of $N$ observed $n$-dimensional signals consists of a linear combination of at most $k$ elementary waveforms drawn from some unknown `dictionary' of size $m$, typically where $k < m \ll N$ (see \cite{Zhang15} for a comprehensive review of this and related models).
...
Approximate solutions to this problem have provided insight into the structure of many signal classes lacking explicit analytic descriptions [cite?]. 
In particular, it has been shown that response properties of simple-cell neurons in mammalian visual cortex emerge from the optimization of a dictionary to represent small patches of natural images \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}, a major advance in computational neuroscience. A curious aspect of this finding is that the latent waveforms (``Gabors'') estimated from data appear to be canonical [cite?]; %\cite{donoho2001can}
...
conditions have recently been proposed (see \cite[Sec.~I-E]{Sun16} for a summary of the state-of-the-art). There are few theorems one can cite to explain the general phenomenon of uniqueness, however.  
...
Applications of our results include:  1) a sufficient diversity of sparse codes sent through a random linear compressive channel are identifiable and 2) an explanation for universal representation of natural signals in sparse coding models from neuroscience.
...
Find a matrix $\mathbf{B} \in \mathbb{R}^{\bar m}$ and $k$-sparse $\mathbf{\bar x}_1, \ldots, \mathbf{\bar x}_N \in \mathbb{R}^m$ that solve:
...
There are other less direct consequences of Thm.~\ref{DeterministicUniquenessTheorem} to stability in sparse linear coding. Consider the optimization problem posed in Prob.~\ref{SLCopt}.
